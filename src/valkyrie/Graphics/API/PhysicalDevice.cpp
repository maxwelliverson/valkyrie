...
namespace valkyrie::Graphics::API{
  namespace Internal{
    namespace {
      ...
      void lazyLoadQueueFamiliesDefault(PhysicalDeviceImpl &impl) noexcept {
        u32 queueFamilySize;
        Core::StaticArray<VkQueueFamilyProperties2, 8> queueFamiliesTmp;
      ...
      void lazyLoadQueueFamiliesWithCheckpoints(PhysicalDeviceImpl &impl) noexcept {
        u32 queueFamilySize;
        Core::StaticArray<VkQueueFamilyCheckpointPropertiesNV, 8> checkpointPropertiesTmp;
        Core::StaticArray<VkQueueFamilyProperties2, 8> queueFamiliesTmp;
        impl.pInterface->getQueueFamilyProperties(impl.handle, &queueFamilySize, nullptr);
        queueFamiliesTmp.resize(queueFamilySize);
  ...
  class PhysicalDevice::Impl : public Internal::PhysicalDeviceImpl{};
...
namespace {
  ...
  using QueueFlags = Core::BitFlags<QueueFlagBits>;
  using PipelineStageFlags = Core::BitFlags<VkPipelineStageFlagBits>;

  enum class PerformanceCounterFlagBits{
    PerformanceImpacting = 0x00000001,
    ConcurrentlyImpacted = 0x00000002,
  };
  using PerformanceCounterFlags = Core::BitFlags<PerformanceCounterFlagBits>;
  ...
  struct ExtensionInfo{
    ...
    valkyrie::Core::Version specVersion;
    bool                    isEnabled;
...
class PhysicalDevice::Impl{
  ...
  mutable Core::FlatSet<ExtensionInfo> extensions;

  Core::FlatSet<std::string, Core::DynamicArray<std::string>> availableExtensions;
  ...
  mutable Core::FlatMap<VkSampleCountFlagBits, VkMultisamplePropertiesEXT> multisampleProperties;
...
public:
  ...
  VkDevice makeDevice(std::span<const VkDeviceQueueCreateInfo> queueInfo) noexcept {
    Core::DynamicArray<const char*> deviceExtensions;

...
#define VK_enumerate_formats(...) VK_foreach(PP_VK_impl_RET_FORMAT, __VA_ARGS__)
  ...
  namespace DXGI{
    ...
    class StatusDomain : public Core::StatusDomain{
      inline constexpr static Core::Uuid uuid{"a16442cf-17f2-4180-a14a-f66f71d63792"};
    public:
      using value_type = RawResult;
      using typename Core::StatusDomain::string_ref;
      using status_type = Core::StatusCode<StatusDomain>;

      Core::StringView name() const noexcept override{
        return VK_string("DirectX Graphics Infrastructure");
      }
      string_ref doMessage(const Core::StatusCode<void>& status) const noexcept override {
      ...
      bool doEquivalent(const Core::StatusCode<void>& a, const Core::StatusCode<void>& b) const noexcept override{
      ...
      const Core::Uuid & id() const noexcept override {
        return uuid;
      }

      inline constexpr static const StatusDomain& get() noexcept;
    };

    inline constexpr static StatusDomain domainInstance{};

    inline constexpr const StatusDomain& StatusDomain::get() noexcept {
      return domainInstance;
    }

    using Status = Core::StatusCode<StatusDomain>;
    using Error  = Core::ErrorCode<StatusDomain>;
    ...
    template <typename T>
    using Result = Core::Maybe<T, StatusDomain>;

    class Output{

      using FormatMap = Core::FlatMap<DXGI_FORMAT, Core::SmallArray<DXGI_MODE_DESC>>;
    ...
    public:
      ...
      Core::Interval<float>     getLuminance() const noexcept {
        return luminance;
      }
      Core::Interval<float>     getFullFrameLuminance() const noexcept {

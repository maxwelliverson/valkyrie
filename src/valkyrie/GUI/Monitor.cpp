...
namespace{
  ...
  public:
    ...
    template <typename T> requires(!CustomObject<T> && requires(std::ostream& iOS, T&& t){
      { iOS << std::forward<T>(t) } -> valkyrie::ExactSameAs<std::ostream&>;
    })
...
namespace valkyrie::GUI{
  ...
  namespace {
    inline Core::SmallArray<Internal::MonitorImpl> systemMonitors{};
    inline Core::SmallArray<Internal::MonitorImpl*> systemMonitorPointers{};
    inline void   findMonitors(){
      ...
      while (EnumDisplayDevices(nullptr, deviceIndex, &displayDevice, 0)){
        ...
        Core::SmallArray<u32, 4> monitorIndices{};

  ...
  namespace Arrays{
    ...
    namespace detail{
      ...
      concept IsMappingType = requires(const M& map){
        ...
        { map.requiredSpanSize() } -> same_as<typename M::index_type>;
    ...
    public:
      ...
      [[nodiscard]] static constexpr bool isAllocated() noexcept { return not_same_as<allocator_type, void>; /*TODO: Change to a static allocator type (see boost for example) */}
...
namespace valkyrie {
  ...
  struct traits::static_array<Array<T, Extents...>> {
    ...
    inline constexpr static const T *data(const Array<T, Extents...> &span) noexcept;
  ...
  requires(ExtentsType::dynamicRank() == 0) struct traits::static_array<Arrays::BasicArrayView<T, ExtentsType, LayoutType, AccessorType>> {
    using element_type = T;
    inline constexpr static T *data(const Arrays::BasicArrayView<T, ExtentsType, LayoutType, AccessorType> &span) noexcept {
      return span.data();
    }
    inline constexpr static size_t size = ExtentsType::totalStaticExtent();
  };
  template<typename T, typename ExtentsType, typename LayoutType, typename AccessorType>
  struct traits::DynamicArray<Arrays::BasicArrayView<T, ExtentsType, LayoutType, AccessorType>> {
    using element_type = T;
    inline constexpr static T *data(const Arrays::BasicArrayView<T, ExtentsType, LayoutType, AccessorType> &span) noexcept {
      return span.data();
    }
    inline constexpr static size_t size(const Arrays::BasicArrayView<T, ExtentsType, LayoutType, AccessorType> &span) noexcept {
  ...
  struct traits::static_array<CArray<T, N>> {
    ...
    inline constexpr static const T *data(const CArray<T, N> &cArray) noexcept {
      return cArray.data();
    }
  ...
  template<typename T, size_t N>
  struct traits::static_array<const T[N]> {

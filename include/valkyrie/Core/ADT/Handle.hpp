...
namespace valkyrie::Policies{
  ...
  struct StrongRefCountedOwnership{
    ...
    inline T*   acquire(Core::IntrusiveReferenceCount<T, Base>* pointer) const noexcept {
    ...
    template <typename T, typename Base>
    inline bool release(Core::IntrusiveReferenceCount<T, Base>* pointer) const noexcept {
    ...
    BasicHandle(T handle, Ownership ownership = {}, Destruction destruction = {}, Nullability nullability = {}) noexcept
      ...
      if constexpr (!Nullability::IsNullable) {
        if (this->nullability.isNull(this->handle))
          panic(GenericError(Code::FailedAssertion), VK_string("NonNullable Handle was initialized with a null handle"));
      }
    }

...
#define PP_VK_impl_DEFINE_ENUM_ENTRY_redirect(Enum) ::valkyrie::Core::Detail::EnumEntry{enum_type::Enum, VK_raw_string(Enum)}
...
#define VK_enum_values(Type, ...)                                                          \
  ...
  inline constexpr static std::string_view scoped_name{::valkyrie::Core::Detail::typeName<enum_type>()};                   \
...
namespace valkyrie::Core{
  ...
  namespace Detail{
    ...
    template <typename E, size_t N>
    class EnumEntryMap{
      using container_type = FlatMap<E, StringLiteral, EnumEntryMapContainer<E, N>>;
      using underlying_type = std::underlying_type_t<E>;
      using enum_type = E;
    public:

      template <typename ...Args> requires(std::same_as<Args, EnumEntry<E>> && ...)
      constexpr EnumEntryMap(Args&& ...args) noexcept
          : entries{ guaranteeUnique, std::in_place, std::forward<Args>(args)... }{}

      constexpr StringLiteral operator[](E value) const noexcept {
  ...
  template <typename E>
  class StatusEnumMap{
    using container_type = FlatMap<E, Detail::StatusEnumInfo, Detail::StatusEnumEntryMapContainer<E>>;
    using underlying_type = std::underlying_type_t<E>;
    using enum_type = E;
...
#define PP_VK_impl_PROCESS_STATUS_ENUM_INFO_redirect_2(val, ...) valkyrie::Core::Detail::StatusEnumEntry<enum_type>{ enum_type::val, valkyrie::Core::Detail::StatusEnumInfo{ PP_VK_impl_STATUS_ENUM_MINI_FOR_EACH(__VA_ARGS__) }}
...
#define VK_status_enum_domain(name_, uuid_) inline constexpr static ::valkyrie::Core::StringView domain_name{VK_raw_string(name_)}; inline constexpr static ::valkyrie::Core::Uuid domain_uuid{uuid_}
#define VK_status_enum_info(...) inline static const ::valkyrie::Core::StatusEnumMap info{ VK_for_each_delimit(PP_VK_impl_PROCESS_STATUS_ENUM_INFO, VK_comma_delimiter, ##__VA_ARGS__) }

  /*template <>
  struct Custom::StatusEnum<ExampleEnum>{

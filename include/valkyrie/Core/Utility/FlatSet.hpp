...
namespace valkyrie::Core{
  ...
  namespace Concepts{
    ...
    concept KeyFor = !std::same_as<std::remove_cvref_t<T>, std::remove_cvref_t<U>> && EqualityComparableWith<T, U>;
    template <typename T, typename U>
    concept OrderedKeyFor = !std::same_as<std::remove_cvref_t<T>, std::remove_cvref_t<U>> && OrderedWith<T, U>;
  }

  inline constexpr static struct GuaranteeUniqueTag{} guaranteeUnique;

  template </*Concepts::Ordered*/typename T, typename Container = SmallArray<T>>
  class FlatSet {
  ...
  public:
    ...
    constexpr FlatSet(I&& first, S&& last) : m_set_(std::forward<I>(first), std::forward<S>(last)){
      priv_prune_();
    }
    ...
    template <std::input_iterator I, std::sentinel_for<I> S> requires(std::convertible_to<std::iter_reference_t<I>, value_type>)
    constexpr FlatSet(GuaranteeUniqueTag, I&& first, S&& last) : m_set_(std::forward<I>(first), std::forward<S>(last)){
      priv_sort_();
    }
    ...
    constexpr FlatSet& operator&=(const FlatSet& Other) noexcept {
    ...
    constexpr maybe_iterator insert(value_type&& Value) noexcept requires(!IsRegisterCopyable<value_type>) {
      //iterator iter = std::ranges::lower_bound(m_set_, Value);
      iterator iter = std::lower_bound(m_set_.begin(), m_set_.end(), Value);
      if (iter != end() && *iter == Value) return std::nullopt;
    ...
    constexpr void swap(FlatSet& Other) noexcept {
    ...
    friend constexpr FlatSet operator&(const FlatSet& A, const FlatSet& B) noexcept {
      FlatSet C;
    ...
    friend constexpr bool operator==(const FlatSet& A, const FlatSet& B) noexcept {
      return std::ranges::equal(A, B);
    }
    friend constexpr auto operator<=>(const FlatSet& A, const FlatSet& B) noexcept {
